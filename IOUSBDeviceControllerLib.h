...
/*!
 ...
 @discussion The controller will drop off USB appearing to the host as if it has been unlugged. After the given 
msecDelay
...
/*!
 ...
 @param      deviceRef The controller object
 @param      enable If true the controller is dropped off the bus and kept off. When false the controller will no 
longer be forced off.
 @result     The status of the call.
 */
CF_EXPORT
...
/*! @function   IOUSBDeviceControllerRegisterArrivalCallback
 ...
 @param      callback The function invoked when the controller arrives. It receives a IOUSBDeviceControllerRef 
annd the caller-provided context. 
 ...
 */
CF_EXPORT
IOReturn IOUSBDeviceControllerRegisterArrivalCallback(IOUSBDeviceArrivalCallback callback, void *context, 
 ...
 */
CF_EXPORT
IOReturn IOUSBDeviceControllerSetDescription(IOUSBDeviceControllerRef device, IOUSBDeviceDescriptionRef 
description)
AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;

/*! @function   IOUSBDeviceControllerSendCommand
 @abstract   Issue a command to the in-kernel usb-device stack
 @discussion This sends a command string and optional parameter object into the kernel. Commands are passed to the 
...
"device", then to the individual interface drivers, until one of those handles it.
 ...
 @param      command A string command. Valid commands are determined by the various in-kernel drivers comprising 
...
/*! @function   IOUSBDeviceDescriptionCreateFromController
 ...
 @discussion This retrieves the currently set description from the kernel's IOUSBDeviceController. It represents 
the full description of the device as
 it is currently presented on the USB. The call can fail if the controller exists but has not et received a 
description.
 ...
 */
CF_EXPORT
IOUSBDeviceDescriptionRef IOUSBDeviceDescriptionCreateFromController(CFAllocatorRef allocator, 
IOUSBDeviceControllerRef)
AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;
...
/*! @function   IOUSBDeviceDescriptionCreateFromDefaults
 ...
 @discussion This retrieves the default description for the device. It describes the main usb functionality 
provided by the device and is what is used for
 a normal system. Currently the description is retrieved from a plist on disk and is keyed to a sysctl that 
...
CF_EXPORT
int IOUSBDeviceDescriptionAppendInterfaceToConfiguration(IOUSBDeviceDescriptionRef devDesc, int config, 
CFStringRef name);
AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;

CF_EXPORT
int IOUSBDeviceDescriptionAppendConfiguration(IOUSBDeviceDescriptionRef devDesc, CFStringRef textDescription, 
UInt8 attributes, UInt8 maxPower);
AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;
...
/*! @function   IOUSBDeviceDescriptionCopyInterfaces
 ...
 @discussion This function returns an array of arrays of strings where each item in the top array corresponds to a 
single configuration and each string is the name of an interface in on the configuration.
